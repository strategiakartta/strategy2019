...
public class MapTypes {
	...
	public static String canRemove(Database database, Base level) {
    	return UtilsDBMapTypes.canRemove(database, level);
	}
	...
	public static void selectMapType(final Main main, final StrategyMap map) {
        ...
        aseta.setStyleName(ValoTheme.BUTTON_SMALL);
        aseta.setEnabled(false);

        final Button poista = new Button("Poista");
        poista.setEnabled(false);
        poista.setStyleName(ValoTheme.BUTTON_SMALL);

        final Button hallinnoi = new Button("Muokkaa");
        hallinnoi.setStyleName(ValoTheme.BUTTON_SMALL);
        ...
        aseta.addClickListener(new Button.ClickListener() {
            ...
            public void buttonClick(ClickEvent event) {
            	...
            	map.setCurrentLevel(main.getDatabase(), newLevel);
	...
	/**
	 * Force the newly created maptype to the the maptype that defines offices.
	 */
	public static ObjectType createMapTypeOfficeDefiner(Main main, Database database, String name,
			boolean linkWithParent, boolean goalSubmap) {
		return UtilsDBMapTypes.createMapType(main != null, database,
				database.getDefaultAdminAccount(), name,
				linkWithParent, goalSubmap,
				true);
	}
	
	/**
	 * Create a new map type that does not define offices
	 */
	public static ObjectType createMapType(Main main, Database database, String name, boolean linkWithParent, boolean goalSubmap) {
		return UtilsDBMapTypes.createMapType(main != null, database,
				database.getDefaultAdminAccount(), name,
				linkWithParent, goalSubmap,
				false);
